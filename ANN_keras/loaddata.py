...
def load_LJdata(file, verbose=1):
    '''
    load LJ training and test data
    Input: 
    ...
    LJ_test      = pd.read_csv(file)
    
    size = LJ_test.shape[1] - 3
    
    #print information about test dataset
    LJ_test_liquid = LJ_test[LJ_test['T']>0.8]
    LJ_test_solid  = LJ_test[LJ_test['T']<=0.8]
    LJ_test_ex     = pd.concat([LJ_test_liquid, LJ_test_solid], axis=0)
    
    n_test_T        = LJ_test_ex['T'].unique().shape[0]
    
    LJ_test_y      = LJ_test_ex['type'].values
    LJ_test_y      = LJ_test_y.reshape(LJ_test_y.shape[0], 1)
    LJ_test_x      = LJ_test_ex.drop(['type','id','T'], axis=1).values
    LJ_test_T      = LJ_test_ex['T'].values
    
    if verbose:
        #print information about training dataset
        print("number of test            samples : ", LJ_test.shape[0])
        print("number of                features : ", size)
        print("number of T                (test) : ", n_test_T)
        print("LJ_test_x data shape              : ", LJ_test_x.shape)
        print("LJ_test_y data shape              : ", LJ_test_y.shape)
        print('-------------------------------------------------------')
    
    return LJ_test_x, LJ_test_y, LJ_test_T, n_test_T
